...
public class DessinateurLiens extends JPanel {
			...
			public void mouseMoved(MouseEvent e) {
				...
				if (dl != dlSurvol) {
					if (dl != null) {
						dl.setSurvol(true);
						repeindre(dl);
						// logger.info("Rectangle survol y,h = " + dl.getRectangleSurvol().y + "," +
						// dl.getRectangleSurvol().height);
					}
					if (dlSurvol != null) {
						...
						if (dl != null && dlSelectionnes.contains(dl)) {
							...
							if ((e.getModifiersEx() & MouseEvent.SHIFT_DOWN_MASK) != MouseEvent.SHIFT_DOWN_MASK) {
								while (dlSelectionnes.size() > 2) {
									DessinLien dl2 = (DessinLien) dlSelectionnes.remove(0); // On
																							// enleve
																							// le
																							// premier
																							// �
	...
	private void dessinerLiens(Graphics g, boolean avecSelections) {
		for (int i = 0; i < dessinsLiens.length; i++) {
			...
			if (dessinLien != null) {
				...
				dessinLien.setSurlignage(model.getClefsLiensSurlignes().contains(
						dessinLien.getLien().getClef()));
	...
	/**
	 * @param largeurReference
	 *            the largeurReference to set
	 */
	...
	/**
	 * @param largeurDessinMax
	 *            the largeurDessinMax to set
	 */
