...
public class InterlocIhm extends JFrame {
    ...
    private static final String BANNIERE = "Interloc by <Megiste> V " + InterlocMain.readInterlocVersion();
    ...
    public static final FileFilter RTF_FILE_FILTER = new FileFilter() {
        ...
        public boolean accept(File f) {
            if (f.isDirectory())
                ...
                return f.getName().toLowerCase().endsWith("rtf") || f.getName().toLowerCase().endsWith("doc")
    ...
    private void afficherInfo(String clef, Object[] values) {
        if(clef==null) labelInfo.setText("");
        else {
            String message = null;
            if (values != null) {
    ...
    protected void changerCharset() throws LectureHOMException, ErreurRecupHistoriqueException {
        ...
        afficherInfo("message.rechargementcharset", new String[] { fichierCourant.getPath(), charsetCourant });
        dessinateur.getModel().setModif(true);
    }

    public void chargeFichier(File selectedFile) throws LectureHOMException, ErreurRecupHistoriqueException {
        ...
        dessinateur.getModel().initLiens(accesseurFichierHOM.getLiensCharges(), accesseurFichierHOM.getSurlignages());
     ...
     * 
     * Icon icon = new ImageIcon((BufferedImage) dessinateur.creerImage());
     * final JLabel label = new JLabel(icon); label.setTransferHandler(new
     * ImageTransferable()); TransferHandler handler =
     * label.getTransferHandler(); Toolkit kit = Toolkit.getDefaultToolkit();
    ...
    private void enregistrerAction(String chars, InterlocAction action, boolean invisible, int quelPanneau) {
    ...
    protected void exporterFichierDansDocWord() throws ErreurExportException {
        if (lastExportTotalSelectedFile != null) {
            ...
            chooserExport.setSelectedFile(new File(fichierCourant.getParentFile(), fichierCourant.getName().replaceAll(
                    "." + AccesseurFichierHOM.SUFFIX_HOM, "." + RTFExporter.SUFFIX_RTF)));
        }
    ...
    protected void exporterFichierEnImage() throws IOException {
        ...
        if (lastExportedPicture.getParentFile().exists()) {
        ...
        String nomBaseFichierImage = fichierCourant.getName().replaceAll("." + AccesseurFichierHOM.SUFFIX_HOM, "");
        ...
        File fichierImage = new File(repertoireImages, nomBaseFichierImage + "." + SUFFIX_JPG);
        int compteur = 1;
        while (fichierImage.exists()) {
            fichierImage = new File(repertoireImages, nomBaseFichierImage + "_" + compteur + "." + SUFFIX_JPG);
            compteur++;
        }

        chooserExport.setSelectedFile(fichierImage);
        int returnVal = chooserExport.showSaveDialog(this);
        if (returnVal == JFileChooser.APPROVE_OPTION) {
            ImageIO.write(rendImage, "jpg", chooserExport.getSelectedFile());
            afficherInfo("message.exportOK", new String[] { chooserExport.getSelectedFile().getPath() });
            ;
            dessinateur.nettoyerSelectionExport();
    ...
    private void exporterPasAPas() throws ErreurExportException {
        ...
        if (lastExportSelectedFile != null) {
            chooserExport.setSelectedFile(new File(lastExportSelectedFile.getParentFile(), filename));
        } else {
        ...
        if (returnVal == JFileChooser.APPROVE_OPTION) {
            ...
            exporteur.exporterPasAPas(chooserExport.getSelectedFile(), dessinateur.getIndiceMin(), dessinateur
                    .getIndiceMax());
            lastExportSelectedFile = chooserExport.getSelectedFile();
            afficherInfo("message.exportppOK", new String[] { chooserExport.getSelectedFile().getPath() });
        }
    ...
    public void init() {
            ...
            public void windowClosing(WindowEvent arg0) {
                if (dessinateur.getModel().isModif()) {
                    int res = JOptionPane.showConfirmDialog(InterlocIhm.this,
                            "Vous n'avez pas sauvé...\nVoulez vous quitter quand même?", "Confirmation",
        ...
        testTailles.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                testTailles.setText("" + testTailles.getSize().width + "," + testTailles.getSize().height);
    ...
    protected void exporterFichierDansDocHTML() throws ErreurExportException {
        ...
        String filename = fichierCourant.getName().replaceAll(
                "." + AccesseurFichierHOM.SUFFIX_HOM, id + "." + DocumentExporter.SUFFIX_HTML);
        
        if (lastExportTotalSelectedFile != null) {
            chooserExport.setSelectedFile(new File(lastExportTotalSelectedFile.getParentFile(), filename));
        } else {
    ...
    private void majTitre() {
        ...
        setTitle(BANNIERE + " - " + fichierCourant.getName() + " " + (dessinateur.getModel().isModif() ? "*" : ""));
    ...
    protected void sauverFichierSous() throws ErreurEcritureException {
        ...
        if (returnVal == JFileChooser.APPROVE_OPTION) {
            ...
            if (fileDest.exists()) {
                int res = JOptionPane.showConfirmDialog((JComponent) this.getContentPane(), "" + fileDest.getName()
                        + " existe. Voulez-vous le remplacer?", "Confirmer", JOptionPane.YES_NO_OPTION);
    ...
    protected void scroll(int scroll) {
        ...
        if (pos.y + scrollPane.getViewport().getSize().height > dessinateur.getSize().height) {
            pos.y = dessinateur.getSize().height - scrollPane.getViewport().getSize().height + 2 * DessinLien.LIGNE;
        }
    ...
    public void traitementErreur(Throwable e) {
        JOptionPane.showInternalMessageDialog(this.getContentPane(), "Erreur de type : " + e.getClass().getName()
